...
const popupAddCard = new PopupWithForm(selectors.cardForm, data => {
    api.createNewCard(data)
        ...
        .catch((error) => {
            handleFormError(error, "Failed to get response from server")
            popupAddCard.resetLoading()
        })
})
...
const popupAvatarEdit = new PopupWithForm(selectors.profileAvatarForm, link => {
    api.setAvatar(link)
        ...
        .catch((error) => {
            handleFormError(error, "Failed to update")
            popupAvatarEdit.resetLoading()
        })
...
function getInitialData() {
    ...
    Promise.all([api.getUserInfo(), api.getInitialCards()])
        ...
        .catch((error) => {
            handleFormError(error, "Failed to get card & user info")
        })
}
...
function handleCardDelete(card) {
    ...
    popupDeleteCard.setAction(() => {
        api.deleteCard(card.cardId)
            ...
            .catch((error) => {
                handleFormError(error, "Failed to delete card")
                popupDeleteCard.resetLoading()
            })
...
function handleCardLike(card) {
    if (card.isLiked()) {
        api.removeLike(card.cardId)
            ...
            .catch((error) => {
                handleFormError(error, "Failed to remove Like")
        ...
        api.addLike(card.cardId)
            .then((res) => card.toggleLike(res.likes))
            .catch((error) => {
                handleFormError(error, "Failed to add a like")
...
function handleFormError(error, defaultErrorText) {
    if (error == "") error = defaultErrorText
    popupWithMessage.open({ title: "Error occurs", subtext: error }, true)
